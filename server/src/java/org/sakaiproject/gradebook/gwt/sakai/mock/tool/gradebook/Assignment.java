/**********************************************************************************
 *
 * $Id: Assignment.java 37479 2007-10-29 18:30:26Z cwen@iupui.edu $
 *
 ***********************************************************************************
 *
...
public class Assignment extends GradableObject {
	...
	public void calculateStatistics(Collection<AssignmentGradeRecord> gradeRecords) {
		...
		if (numScored == 0) {
			...
			if(!ungraded && pointsPossible > 0)
			{
				mean = new Double(total.divide(bdNumScored, MATH_CONTEXT).doubleValue());
			}
			else
			{
				mean = null;
			}
			averageTotal = new Double(pointsTotal.divide(bdNumScored, MATH_CONTEXT).doubleValue());
		}
	}
