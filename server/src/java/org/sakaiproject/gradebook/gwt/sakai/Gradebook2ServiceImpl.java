...
public class Gradebook2ServiceImpl implements Gradebook2Service {
	 ...
	 */
	public ItemModel addItemCategory(String gradebookUid, Long gradebookId,
			ItemModel item) throws BusinessRuleException {

		ActionRecord actionRecord = new ActionRecord(gradebookUid, gradebookId,
				EntityType.CATEGORY.name(), ActionType.CREATE.name());
	...
	public PermissionEntryModel createPermissionEntry(Long gradebookId,
			PermissionEntryModel permissionEntryModel) {
	...
	public PermissionEntryModel deletePermissionEntry(Long gradebookId,
			PermissionEntryModel permissionEntryModel) {
	...
	public SpreadsheetModel createOrUpdateSpreadsheet(String gradebookUid,
			SpreadsheetModel spreadsheetModel) throws InvalidInputException {
			...
			for (ItemModel item : headers) {
				...
				if (id != null) {
					...
					if (id.startsWith("NEW:")) {
						...
						Assignment assignment = gbService.getAssignment(Long
								.valueOf(id));
						...
						if (weight != null
								&& assignment.getAssignmentWeighting() != null
								&& !weight.equals(assignment
				...
				if (newCategoryIdSet != null && !newCategoryIdSet.isEmpty()) {
					...
					for (Long categoryId : newCategoryIdSet) {
						...
						if (businessLogic
							...
							recalculateAssignmentWeights(category,
									Boolean.FALSE, assigns);
					}
		...
		realmIds[0] = new StringBuffer().append("/site/").append(siteId)
				.toString();

		List<AssignmentGradeRecord> allGradeRecords = gbService
				.getAllAssignmentGradeRecords(gradebookId, realmIds,
						learnerRoleNames);
		...
		if (allGradeRecords != null) {
			for (AssignmentGradeRecord gradeRecord : allGradeRecords) {
				...
				Map<Long, AssignmentGradeRecord> studentMap = userRecord
						.getGradeRecordMap();
				if (studentMap == null) {
		...
		if (idKeySet != null) {
			for (StudentModel student : spreadsheetModel.getRows()) {
				UserRecord userRecord = userRecordMap.get(student
						.getIdentifier());
				...
				if (userRecord != null) {
					if (userRecord.getDisplayName() == null)
						...
						builder.append(userRecord.getDisplayName())
								.append(": ");
				} else {
				...
				Map<Long, AssignmentGradeRecord> gradeRecordMap = userRecord == null ? null
						: userRecord.getGradeRecordMap();
				...
				for (String id : idKeySet) {
					...
					if (gradeRecordMap != null)
						assignmentGradeRecord = gradeRecordMap.get(assignment
								.getId()); // gbService.getAssignmentGradeRecordForAssignmentForStudent(assignment,
					...
					try {
						scoreItem(gradebook, assignment, assignmentGradeRecord,
								student.getIdentifier(), value, true, false);
						...
						String failedProperty = new StringBuilder().append(
								assignment.getId()).append(
								StudentModel.FAILED_FLAG).toString();
						student.set(failedProperty, e.getMessage());
						log.warn("Failed to score numeric item for "
								+ student.getIdentifier() + " and item "
								+ assignment.getId() + " to " + value);
		...
		List<Assignment> assignments = gbService.getAssignments(gradebook
				.getId());
		List<Category> categories = null;
		if (hasCategories)
			categories = getCategoriesWithAssignments(gradebook.getId(),
					assignments, true);
		spreadsheetModel.setGradebookItemModel(getItemModel(gradebook,
				assignments, categories, null, null));

		return spreadsheetModel;
	}

	private List<UserRecord> doSearchUsers(String searchString,
			List<String> studentUids, Map<String, UserRecord> userRecordMap) {
		...
		if (users != null) {
			for (User user : users) {
				...
				String sortName = new StringBuilder().append(
						lastName.toUpperCase()).append("::").append(
						firstName.toUpperCase()).toString();
	...
	private List<UserRecord> doSearchAndSortUserRecords(Gradebook gradebook, List<Assignment> assignments, List<Category> categories,
		...
		if (config != null && config.getSortInfo() != null
				&& config.getSortInfo().getSortField() != null) {
		...
		boolean isDescending = config != null && config.getSortInfo() != null
				&& config.getSortInfo().getSortDir() == SortDir.DESC;
		...
		if (sortColumnKey != null) {
			...
			case EMAIL:
				if (userRecords == null) {
					userRecords = doSearchUsers(searchString, studentUids,
							userRecordMap);
				}
				break;
			case COURSE_GRADE:
			...
			case ASSIGNMENT:
				if (userRecords == null) {
					userRecords = new ArrayList<UserRecord>(userRecordMap
							.values());
				}
				break;
			}
			...
			case COURSE_GRADE:
				...
				for (UserRecord record : userRecords) {
					record
							.setDisplayGrade(getDisplayGrade(gradebook, record
									.getUserUid(), record
									.getCourseGradeRecord(), assignments,
									categories, record.getGradeRecordMap()));
			...
			case ASSIGNMENT:
				...
				comparator = new AssignmentComparator(assignmentId,
						isDescending);
				break;
			}
	...
	public StudentModel excuseNumericItem(String gradebookUid,
			StudentModel student, String id, Boolean value,
			Boolean previousValue) throws InvalidInputException {
		...
		List<AssignmentGradeRecord> gradeRecords = gbService
				.getAssignmentGradeRecordsForStudent(gradebook.getId(), student
						.getIdentifier());
		...
		gradeRecords = gbService.getAssignmentGradeRecordsForStudent(gradebook
				.getId(), student.getIdentifier());
	...
	public List<UserDereference> findAllUserDereferences() {
		...
		List<UserDereference> dereferences = gbService.getUserDereferences(
				realmIds, "sortName", null, null, -1, -1, true,
				learnerRoleNames);
	...
	public <X extends BaseModel> PagingLoadResult<X> getActionHistory(
			String gradebookUid, PagingLoadConfig config) {

		Integer size = gbService.getActionRecordSize(gradebookUid);
		List<ActionRecord> actionRecords = gbService.getActionRecords(
				gradebookUid, config.getOffset(), config.getLimit());
		List<X> models = new ArrayList<X>();

		for (ActionRecord actionRecord : actionRecords) {
			UserEntityAction actionModel = null;
			try {
				UserEntityAction.ActionType actionType = UserEntityAction.ActionType
						.valueOf(actionRecord.getActionType());
				UserEntityAction.EntityType entityType = UserEntityAction.EntityType
						.valueOf(actionRecord.getEntityType());
				switch (actionType) {
				case CREATE:
				...
				if (actionRecord.getParentId() != null)
					actionModel.setParentId(Long.valueOf(actionRecord
							.getParentId()));
				...
				if (userService != null && actionRecord.getGraderId() != null) {
					...
					try {
						User user = userService.getUser(actionRecord
								.getGraderId());
						actionModel.setGraderName(user.getDisplayName());
					} catch (UserNotDefinedException e) {
						log.warn("Unable to find grader name for "
								+ actionRecord.getGraderId(), e);
					}
				...
				log.warn("Failed to retrieve history record for "
						+ actionRecord.getId());
			}
		}

		return new BasePagingLoadResult<X>(models, config.getOffset(), size
				.intValue());
	}
	...
	public ApplicationModel getApplicationModel(String... gradebookUids) {
		...
		if (configService != null) {
			String url = configService
					.getString(AppConstants.HELP_URL_CONFIG_ID);
	...
	public <X extends BaseModel> ListLoadResult<X> getGradeEvents(
		...
		Map<GradableObject, List<GradingEvent>> map = gbService
				.getGradingEventsForStudent(studentId, gradableObjects);
		...
		if (events != null) {
			Collections.sort(events, new Comparator<GradingEvent>() {
				...
				public int compare(GradingEvent o1, GradingEvent o2) {
					...
					if (o2.getDateGraded() == null
							|| o1.getDateGraded() == null)
						return 0;
	...
	public <X extends BaseModel> PagingLoadResult<X> getSections(String gradebookUid,
		...
		return new BasePagingLoadResult<X>(sections, config.getOffset(),
				viewableSections.size());
	}

	public <X extends BaseModel> ListLoadResult<X> getSelectedGradeMapping(
			String gradebookUid) {
	...
	public <X extends BaseModel> ListLoadResult<X> getStatistics(String gradebookUid, Long gradebookId) {
		...
		List<AssignmentGradeRecord> allGradeRecords = gbService
				.getAllAssignmentGradeRecords(gradebook.getId(), realmIds,
						learnerRoleNames);
		...
		if (allGradeRecords != null) {
			for (AssignmentGradeRecord gradeRecord : allGradeRecords) {
				gradeRecord.setUserAbleToView(true);
				String studentUid = gradeRecord.getStudentId();
				Map<Long, AssignmentGradeRecord> studentMap = studentGradeRecordMap
						.get(studentUid);
				if (studentMap == null) {
					studentMap = new HashMap<Long, AssignmentGradeRecord>();
				}
		...
		for (String studentUid : gradedStudentUids) {
			...
			case GradebookService.CATEGORY_TYPE_NO_CATEGORY:
				courseGrade = gradeCalculations.getCourseGrade(gradebook,
						assignments, studentMap);
				break;
			default:
				courseGrade = gradeCalculations.getCourseGrade(gradebook,
						categories, studentMap);
			}
	...
	public <X extends BaseModel> PagingLoadResult<X> getStudentRows(
			String gradebookUid, Long gradebookId, PagingLoadConfig config,
			Boolean includeExportCourseManagementId) {

		boolean includeCMId = DataTypeConversionUtil
				.checkBoolean(includeExportCourseManagementId);
		...
		if (config != null) {
			...
			if (config.getSortInfo() != null
				...
				for (StudentModel.Key key : EnumSet
						.allOf(StudentModel.Key.class)) {
					if (columnId.equals(key.name())) {
						sortColumnKey = key;
						break;
					}
			...
			if (config instanceof MultiGradeLoadConfig) {
				...
				searchCriteria = ((MultiGradeLoadConfig) config)
						.getSearchString();
		...
		Collection<Group> groups = site == null ? new ArrayList<Group>() : site
				.getGroups();
		...
		if (!authorizedGroups.isEmpty())
			realmGroupIds = authorizedGroups
					.toArray(new String[authorizedGroups.size()]);
		...
		if (sortColumnKey != null) {
			...
			case EMAIL:
				...
				userRecords = findLearnerRecordPage(gradebook, site, realmIds,
						groupReferences, groupReferenceMap, sortField,
						searchField, searchCriteria, offset, limit,
						!isDescending, includeCMId);
				totalUsers = gbService.getUserCountForSite(realmIds, sortField,
						searchField, searchCriteria, learnerRoleNames);

				int startRow = config == null ? 0 : config.getOffset();

				List<FixedColumnModel> columns = getColumns();

				rows = new ArrayList<X>(userRecords == null ? 0 : userRecords
						.size());

				// We only want to populate the rowData and rowValues for the
				// requested rows
				for (UserRecord userRecord : userRecords) {
					rows.add((X) buildStudentRow(gradebook, userRecord,
							columns, assignments, categories));
				}
			...
			case ASSIGNMENT:
				...
				userRecords = findLearnerRecordPage(gradebook, site, realmIds,
						groupReferences, groupReferenceMap, null, searchField,
						searchCriteria, -1, -1, !isDescending, includeCMId);

				Map<String, UserRecord> userRecordMap = new HashMap<String, UserRecord>(); // findStudentRecords(gradebookUid,
																							// gradebookId,
																							// site,
																							// sectionUuid);
				...
				List<String> studentUids = new ArrayList<String>(userRecordMap
						.keySet());

				// Map<String, Map<Long, AssignmentGradeRecord>>
				// allGradeRecordsMap = new HashMap<String, Map<Long,
				// AssignmentGradeRecord>>();

				/*
				 * List<AssignmentGradeRecord> allGradeRecords =
				 * gbService.getAllAssignmentGradeRecords(gradebookId,
				 * studentUids); // List<AssignmentGradeRecord> allGradeRecords
				 * = gbService.getAllAssignmentGradeRecords(gradebookId, siteId,
				 * sectionUuid, learnerRoleNames);
				 * 
				 * 
				 * if (allGradeRecords != null) { for (AssignmentGradeRecord
				 * gradeRecord : allGradeRecords) {
				 * gradeRecord.setUserAbleToView(true); String studentUid =
				 * gradeRecord.getStudentId(); UserRecord userRecord =
				 * userRecordMap.get(studentUid); Map<Long,
				 * AssignmentGradeRecord> studentMap =
				 * userRecord.getGradeRecordMap(); if (studentMap == null) {
				 * studentMap = new HashMap<Long, AssignmentGradeRecord>(); }
				 * GradableObject go = gradeRecord.getGradableObject();
				 * studentMap.put(go.getId(), gradeRecord);
				 * 
				 ...
				 * gbService.getAllCourseGradeRecords(gradebook);
				 * 
				 * if (courseGradeRecords != null) { for (CourseGradeRecord
				 * courseGradeRecord : courseGradeRecords) { String studentUid =
				 * courseGradeRecord.getStudentId(); UserRecord userRecord =
				 * userRecordMap.get(studentUid); if (userRecord != null)
				 * userRecord.setCourseGradeRecord(courseGradeRecord); else
				 * log.warn("Looking up user record for " + studentUid +
				 * " failed." ); } }
				 * 
				 * List<Comment> comments = gbService.getComments(gradebookId);
				 * 
				 * if (comments != null) { for (Comment comment : comments) {
				 * String studentUid = comment.getStudentId(); UserRecord
				 * userRecord = userRecordMap.get(studentUid); if (userRecord !=
				 * null) { Map<Long, Comment> commentMap =
				 * userRecord.getCommentMap(); if (commentMap == null)
				...
				userRecords = doSearchAndSortUserRecords(gradebook,
						assignments, categories, studentUids, userRecordMap,
						config);
		...
		for (int row = startRow; row < lastRow; row++) {
			...
			if (!userRecord.isPopulated()) {
				...
				try {
					...
					log.error("No sakai user defined for this member '"
							+ userRecord.getUserUid() + "'", e);
				}
			}

			rows.add((X) buildStudentRow(gradebook, userRecord, columns,
	...
	public SubmissionVerificationModel getSubmissionVerification(
			String gradebookUid, Long gradebookId) {
	...
	public StudentModel scoreNumericItem(String gradebookUid,
			StudentModel student, String assignmentId, Double value,
		...
		ActionRecord actionRecord = new ActionRecord(gradebook.getUid(),
				gradebook.getId(), EntityType.GRADE_RECORD.name(),
				ActionType.GRADED.name());
		actionRecord.setEntityName(new StringBuilder().append(
				student.getDisplayName()).append(" : ").append(
		...
		scoreItem(gradebook, assignment, assignmentGradeRecord, student
				.getIdentifier(), value, false, false);
	...
	public StudentModel scoreTextItem(String gradebookUid,
			StudentModel student, String property, String value,
			String previousValue) throws InvalidInputException {
		...
		Gradebook gradebook = gbService.getGradebook(gradebookUid);
		CourseGradeRecord courseGradeRecord = gbService
				.getStudentCourseGradeRecord(gradebook, student.getIdentifier());
		...
		List<AssignmentGradeRecord> records = gbService
				.getAssignmentGradeRecordsForStudent(gradebook.getId(), student
						.getIdentifier());
		...
		String freshCourseGrade = getDisplayGrade(gradebook, student
				.getIdentifier(), courseGradeRecord, assignments, categories,
				studentGradeMap);// requestCourseGrade(gradebookUid,
									// student.getIdentifier());
		student.set(StudentModel.Key.GRADE_OVERRIDE.name(), courseGradeRecord
				.getEnteredGrade());
	...
	public void submitFinalGrade(List<Map<Column, String>> studentDataList,
			String gradebookUid, HttpServletRequest request,
			HttpServletResponse response) {
		advisor.submitFinalGrade(studentDataList, gradebookUid, request,
				response);
	}
	 ...
	 * @throws InvalidInputException
	 */
	public ItemModel updateItemModel(ItemModel item)
			throws InvalidInputException {
		...
		try {
			...
			boolean wasExtraCredit = DataTypeConversionUtil
					.checkBoolean(assignment.isExtraCredit());
			boolean isExtraCredit = DataTypeConversionUtil.checkBoolean(item
					.getExtraCredit());
			...
			havePointsChanged = points != null && oldPoints != null
					&& points.compareTo(oldPoints) != 0;
			...
			boolean isUnweighted = !convertBoolean(item.getIncluded())
					.booleanValue();
			boolean wasUnweighted = DataTypeConversionUtil
					.checkBoolean(assignment.isUnweighted());
			...
			if (!isUnweighted && !isRemoved) {
				// Business rule #2
				assignment.setAssignmentWeighting(gradeCalculations
						.calculateItemWeightAsPercentage(newAssignmentWeight,
								points));
			} else {
			...
			if (hasCategories && category != null) {
				boolean isCategoryIncluded = !DataTypeConversionUtil
						.checkBoolean(category.isUnweighted());
				assignments = gbService.getAssignmentsForCategory(category
						.getId());

				// Business rule #12
				businessLogic.applyCannotUnremoveItemWithRemovedCategory(
						isRemoved, category);

				// Business rule #5
				businessLogic.applyNoDuplicateItemNamesWithinCategoryRule(item
						.getCategoryId(), item.getName(), assignment.getId(),
						assignments);

				// Business rule #6
				businessLogic.applyCannotIncludeDeletedItemRule(wasRemoved
						&& isRemoved, category.isRemoved(), isUnweighted);

				// Business rule #11
				businessLogic.applyCannotIncludeItemFromUnincludedCategoryRule(
						isCategoryIncluded, !isUnweighted, !wasUnweighted);
				...
				// Business rule #3
				businessLogic.applyNoDuplicateItemNamesRule(gradebook.getId(),
						item.getName(), assignment.getId(), assignments);
		...
		if (hasCategories && oldCategory != null) {
			assignments = gbService.getAssignments(gradebook.getId());
			List<Category> categories = getCategoriesWithAssignments(gradebook
					.getId(), assignments, true);
		...
		ItemModel categoryItemModel = getItemModelsForCategory(category, item
				.getParent(), assignment.getId());
	...
	public <X extends BaseModel> List<X> updateGradeScaleField(
			String gradebookUid, Object value, String affectedLetterGrade) {
	...
	public <X extends BaseModel> ListLoadResult<X> getUsers() {
		...
		for (ParticipationRecord participationRecord : participationList) {
			...
			org.sakaiproject.section.api.coursemanagement.User user = participationRecord
					.getUser();
			UserModel userModel = new UserModel(user.getUserUid(), user
					.getDisplayName());
	...
	private Map<String, Object> appendItemData(Long assignmentId,
			Map<String, Object> cellMap, UserRecord userRecord,
			Gradebook gradebook) {
		...
		if (studentGradeMap != null) {
			...
			if (gradeRecord != null) {
				boolean isExcused = gradeRecord.isExcluded() != null
						&& gradeRecord.isExcluded().booleanValue();
				boolean isDropped = gradeRecord.isDropped() != null
						&& gradeRecord.isDropped().booleanValue();

				if (isDropped || isExcused)
					cellMap.put(concat(id, StudentModel.DROP_FLAG),
							Boolean.TRUE);
				...
				case GradebookService.GRADE_TYPE_POINTS:
					...
					break;
				case GradebookService.GRADE_TYPE_PERCENTAGE:
					BigDecimal percentage = gradeCalculations
							.getPointsEarnedAsPercent((Assignment) gradeRecord
									.getGradableObject(), gradeRecord);
					Double percentageDouble = percentage == null ? null
							: Double.valueOf(percentage.doubleValue());
	...
	private StudentModel buildStudentRow(Gradebook gradebook,
			UserRecord userRecord, List<FixedColumnModel> columns,
			List<Assignment> assignments, List<Category> categories) {

		Map<Long, AssignmentGradeRecord> studentGradeMap = userRecord
				.getGradeRecordMap();
		...
		cellMap.put(StudentModel.Key.EXPORT_CM_ID.name(), userRecord
				.getExportCourseManagemntId());
		cellMap.put(StudentModel.Key.EXPORT_USER_ID.name(), userRecord
				.getExportUserId());
		cellMap.put(StudentModel.Key.FINAL_GRADE_USER_ID.name(), userRecord
				.getFinalGradeUserId());
		...
		if (userRecord.isCalculated())
			...
			displayGrade = getDisplayGrade(gradebook, userRecord.getUserUid(),
					courseGradeRecord, assignments, categories, studentGradeMap);

		if (columns != null) {
			for (FixedColumnModel column : columns) {
				StudentModel.Key key = StudentModel.Key
						.valueOf(column.getKey());
				switch (key) {
				case DISPLAY_ID:
					cellMap.put(StudentModel.Key.DISPLAY_ID.name(), userRecord
							.getDisplayId());
					break;
				case DISPLAY_NAME:
					...
					cellMap.put(StudentModel.Key.DISPLAY_NAME.name(),
							displayName);
					cellMap.put(StudentModel.Key.LAST_NAME_FIRST.name(),
							userRecord.getLastNameFirst());
					cellMap.put(StudentModel.Key.EMAIL.name(), userRecord
							.getEmail());
					break;
				case SECTION:
					cellMap.put(StudentModel.Key.SECTION.name(), userRecord
							.getSectionTitle());
					break;
				case COURSE_GRADE:
					if (displayGrade != null)
						cellMap.put(StudentModel.Key.COURSE_GRADE.name(),
								displayGrade);
					break;
				case GRADE_OVERRIDE:
					cellMap.put(StudentModel.Key.GRADE_OVERRIDE.name(),
							enteredGrade);
					break;
				}
				;
		...
		if (assignments != null) {
			...
			for (Assignment assignment : assignments) {
				cellMap = appendItemData(assignment.getId(), cellMap,
						userRecord, gradebook);
			}
	...
	private void calculateItemCategoryPercent(Gradebook gradebook,
			Category category, ItemModel gradebookItemModel,
			ItemModel categoryItemModel, List<Assignment> assignments,
			Long assignmentId) {
		...
		if (assignments != null) {
			for (Assignment a : assignments) {
				double assignmentCategoryPercent = a.getAssignmentWeighting() == null ? 0.0
						: a.getAssignmentWeighting().doubleValue() * 100.0;
				BigDecimal points = BigDecimal.valueOf(a.getPointsPossible()
						.doubleValue());

				boolean isRemoved = a.isRemoved();
				boolean isExtraCredit = a.isExtraCredit() != null
						&& a.isExtraCredit().booleanValue();
			...
			for (Assignment a : assignments) {
				...
				if (!isUnweighted) {
					double w = a == null || a.getAssignmentWeighting() == null ? 0d
							: a.getAssignmentWeighting().doubleValue();
					BigDecimal assignmentWeight = BigDecimal.valueOf(w);
					courseGradePercent = gradeCalculations
							.calculateItemGradePercent(percentGrade,
									percentCategorySum, assignmentWeight);
				}

				ItemModel assignmentItemModel = createItemModel(category, a,
						courseGradePercent);
		...
		if (categoryItemModel != null) {
			categoryItemModel.setPercentCategory(Double
					.valueOf(percentCategorySum.doubleValue()));
			categoryItemModel
					.setPoints(Double.valueOf(pointsSum.doubleValue()));
		}
	}
	...
	private GradebookModel createGradebookModel(Gradebook gradebook, List<Assignment> assignments, List<Category> categories, boolean isNewGradebook) {
		...
		if (userService != null) {
			...
			if (user != null) {
				...
				if (isSingleUserView) {
					...
					try {
						...
						Collection<Group> groups = site
								.getGroupsWithMember(user.getId());
						if (!groups.isEmpty()) {
							for (Group group : groups) {
					...
					List<AssignmentGradeRecord> records = gbService
							.getAssignmentGradeRecordsForStudent(gradebook
									.getId(), userRecord.getUserUid());
					...
					model.setUserAsStudent(buildStudentRow(gradebook,
							userRecord, columns, assignments, categories));
				}
			...
			List<UserRecord> userRecords = findLearnerRecordPage(gradebook,
					getSite(), realmIds, null, null, null, null, null, -1, -1,
					true, false);
			...
			if (userRecords != null && userRecords.size() > 0) {
				...
				model.setUserAsStudent(buildStudentRow(gradebook, userRecord,
						columns, assignments, categories));
			}
		}
	...
	private ItemModel createItemModel(Gradebook gradebook, Category category,
		...
		model.setExtraCredit(category.isExtraCredit() == null ? Boolean.FALSE
				: category.isExtraCredit());
		model.setIncluded(Boolean.valueOf(isIncluded));
		model.setDropLowest(category.getDrop_lowest() == 0 ? null : Integer
				.valueOf(category.getDrop_lowest()));
	...
	private ItemModel createItemModel(Category category, Assignment assignment,
			BigDecimal percentCourseGrade) {

		ItemModel model = new ItemModel();

		double assignmentWeight = assignment.getAssignmentWeighting() == null ? 0d
				: assignment.getAssignmentWeighting().doubleValue() * 100.0;
		Boolean isAssignmentIncluded = assignment.isUnweighted() == null ? Boolean.TRUE
				: Boolean.valueOf(!assignment.isUnweighted().booleanValue());
		Boolean isAssignmentExtraCredit = assignment.isExtraCredit() == null ? Boolean.FALSE
		...
		if (percentCourseGrade == null && hasCategories && category != null) {
			...
			boolean isIncluded = category.isUnweighted() == null ? true
					: !category.isUnweighted().booleanValue();

			double sum = 0d;
			if (assignments != null && isIncluded) {
				for (Assignment a : assignments) {
					double assignWeight = a.getAssignmentWeighting() == null ? 0.0
							: a.getAssignmentWeighting().doubleValue() * 100.0;
					boolean isExtraCredit = a.isExtraCredit() != null
							&& a.isExtraCredit().booleanValue();
					boolean isUnweighted = a.isUnweighted() != null
			...
			percentCourseGrade = new BigDecimal(String.valueOf(Double
					.valueOf(sum)));
		}

		model.setPercentCategory(Double.valueOf(assignmentWeight));
		model.setPercentCourseGrade(Double.valueOf(percentCourseGrade
				.doubleValue()));
	...
	private CommentModel createOrUpdateCommentModel(CommentModel model,
			Comment comment) {
		...
		if (userService != null) {
			try {
				...
				log.warn("Couldn't find the grader for "
						+ comment.getGraderId());
			}
		}
	...
	private GradeEventModel createOrUpdateGradeEventModel(
			GradeEventModel model, GradingEvent event) {
	...
	private GradebookModel createOrRetrieveGradebookModel(String gradebookUid) {
		...
		try {
			...
			// If it doesn't exist, then create it
			if (frameworkService != null) {
				frameworkService.addGradebook(gradebookUid,
	...
	private synchronized Category findDefaultCategory(Long gradebookId) {
		...
		if (categories != null && !categories.isEmpty()) {
			...
			for (Category category : categories) {
				if (category.getName().equalsIgnoreCase(
						AppConstants.DEFAULT_CATEGORY_NAME)) {
		...
		// exception here instead, since it means we weren't able to
		// TODO: create the category for some reason -- but that probably would
		// throw an exception anyway, so...
		if (defaultCategoryId != null) {
	...
	protected List<UserRecord> findLearnerRecordPage(Gradebook gradebook,
			Site site, String[] realmIds, List<String> groupReferences,
			Map<String, Group> groupReferenceMap, String sortField,
			String searchField, String searchCriteria, int offset, int limit,
			boolean isAscending, boolean includeCMId) {

		String[] learnerRoleKeys = advisor.getLearnerRoleNames();
		verifyUserDataIsUpToDate(site, learnerRoleKeys);

		List<UserDereference> dereferences = gbService.getUserDereferences(
				realmIds, sortField, searchField, searchCriteria, offset,
				limit, isAscending, learnerRoleKeys);
		List<AssignmentGradeRecord> allGradeRecords = gbService
				.getAllAssignmentGradeRecords(gradebook.getId(), realmIds,
						learnerRoleKeys);
		Map<String, Map<Long, AssignmentGradeRecord>> studentGradeRecordMap = new HashMap<String, Map<Long, AssignmentGradeRecord>>();
		...
		List<CourseGradeRecord> courseGradeRecords = gbService
				.getAllCourseGradeRecords(gradebook.getId(), realmIds,
						sortField, searchField, searchCriteria, offset, limit,
						isAscending, learnerRoleKeys);
		...
		List<Comment> comments = gbService.getComments(gradebook.getId(),
				realmIds, learnerRoleKeys, sortField, searchField,
				searchCriteria, offset, limit, isAscending);
		...
		if (comments != null) {
			for (Comment comment : comments) {
				...
				Map<Long, Comment> commentMap = studentItemCommentMap
						.get(studentUid);
				if (commentMap == null)
					commentMap = new HashMap<Long, Comment>();
		...
		List<Object[]> tuples = gbService.getUserGroupReferences(
				groupReferences, learnerRoleKeys);
		...
		if (tuples != null) {
			for (Object[] tuple : tuples) {
				...
				if (includeCMId && advisor.isExportCourseManagementIdByGroup()) {
					List<String> enrollmentSetEids = advisor
							.getExportCourseManagementSetEids(group);
					if (enrollmentSetEids != null)
						groupEnrollmentSetEidsMap.put(group.getId(),
								enrollmentSetEids);
				}
		...
		if (dereferences != null) {
			for (UserDereference dereference : dereferences) {
				UserRecord userRecord = new UserRecord(
						dereference.getUserUid(), dereference.getEid(),
						dereference.getDisplayId(), dereference
								.getDisplayName(), dereference
								.getLastNameFirst(), dereference.getSortName(),
						dereference.getEmail());

				Map<Long, AssignmentGradeRecord> gradeRecordMap = studentGradeRecordMap
						.get(dereference.getUserUid());
				userRecord.setGradeRecordMap(gradeRecordMap);
				CourseGradeRecord courseGradeRecord = studentCourseGradeRecordMap
						.get(dereference.getUserUid());
				...
				Set<Group> userGroupSet = userGroupMap.get(userRecord
						.getUserUid());
				if (userGroupSet != null) {
					...
					for (Iterator<Group> groupIter = userGroups.iterator(); groupIter
							.hasNext();) {
						Group group = groupIter.next();
						groupTitles.append(group.getTitle());

						if (includeCMId) {
							List<String> enrollmentSetEids = groupEnrollmentSetEidsMap
									.get(group.getId());
							courseManagementIds.append(advisor
									.getExportCourseManagementId(userRecord
											.getUserEid(), group,
											enrollmentSetEids));
						}
					...
					if (includeCMId)
						userRecord
								.setExportCourseManagemntId(courseManagementIds
										.toString());
				}
				if (includeCMId) {
					userRecord.setExportUserId(advisor
							.getExportUserId(dereference));
					userRecord.setFinalGradeUserId(advisor
							.getFinalGradeUserId(dereference));
				}
	...
	protected List<User> findAllMembers(Site site, String[] learnerRoleKeys) {
		...
		if (site != null) {
			List<String> userUids = gbService.getFullUserListForSite(site
					.getId(), learnerRoleKeys);
	...
	private List<Category> getCategoriesWithAssignments(Long gradebookId,
			List<Assignment> assignments, boolean includeEmpty) {
			...
			for (Assignment assignment : assignments) {
				...
				if (assignment.getCategory() != null)
					category = categoryMap
							.get(assignment.getCategory().getId());
	...
	private String getDisplayGrade(Gradebook gradebook, String studentUid,
			CourseGradeRecord courseGradeRecord, List<Assignment> assignments,
			List<Category> categories,
			Map<Long, AssignmentGradeRecord> studentGradeMap) {
		...
		switch (gradebook.getCategory_type()) {
		case GradebookService.CATEGORY_TYPE_NO_CATEGORY:
			autoCalculatedGrade = gradeCalculations.getCourseGrade(gradebook,
					assignments, studentGradeMap);
			break;
		default:
			autoCalculatedGrade = gradeCalculations.getCourseGrade(gradebook,
		...
		if (gbService.isStudentMissingScores(gradebook.getId(), studentUid,
				hasCategories))
			missingGradesMarker = "***";
	...
	private ItemModel getItemModel(Gradebook gradebook,
			List<Assignment> assignments, List<Category> categories,
			Long categoryId, Long assignmentId) {
		...
		if (isNotInCategoryMode) {
			calculateItemCategoryPercent(gradebook, null, gradebookItemModel,
					null, assignments, assignmentId);
			...
			if (categories != null) {
				...
				for (Category category : categories) {
					boolean isExtraCredit = category.isExtraCredit() != null
							&& category.isExtraCredit().booleanValue();
					boolean isUnweighted = category.isUnweighted() != null
							&& category.isUnweighted().booleanValue();

					if (!category.isRemoved() || isNotInCategoryMode) {
						double categoryWeight = category.getWeight() == null ? 0d
								: category.getWeight().doubleValue() * 100d;

						List<Assignment> items = category.getAssignmentList();
						ItemModel categoryItemModel = createItemModel(
								gradebook, category, items);
						...
						calculateItemCategoryPercent(gradebook, category,
								gradebookItemModel, categoryItemModel, items,
								assignmentId);

						double categoryPoints = categoryItemModel.getPoints() == null ? 0d
								: categoryItemModel.getPoints().doubleValue();

						if (!isExtraCredit && !isUnweighted) {
							gradebookWeightSum = gradebookWeightSum
									.add(BigDecimal.valueOf(categoryWeight));
							gradebookPointsSum = gradebookPointsSum
									.add(BigDecimal.valueOf(categoryPoints));
						}

					}
				}
				gradebookItemModel.setPoints(Double.valueOf(gradebookPointsSum
						.doubleValue()));
				gradebookItemModel.setPercentCourseGrade(Double
						.valueOf(gradebookWeightSum.doubleValue()));
			}
		}
	...
	private ItemModel getItemModelsForCategory(Category category,
			ItemModel gradebookItemModel, Long assignmentId) {
		if (category == null)
			return null;
		...
		calculateItemCategoryPercent(gradebook, category, gradebookItemModel,
	...
	private void recalculateAssignmentGradeRecords(Assignment assignment,
		...
		List<AssignmentGradeRecord> gradeRecords = gbService
				.getAssignmentGradeRecords(assignment);
		...
		if (gradeRecords != null) {
			for (AssignmentGradeRecord gradeRecord : gradeRecords) {
				if (gradeRecord.getPointsEarned() != null) {
					BigDecimal newPoints = gradeCalculations.getNewPointsGrade(
							gradeRecord.getPointsEarned(), value, startValue);
					gradeRecord.setPointsEarned(Double.valueOf(newPoints
							.doubleValue()));
			...
			if (!updatedRecords.isEmpty()) {
				gbService.updateAssignmentGradeRecords(assignment,
						updatedRecords);
			}
		}
	}

	private List<Assignment> recalculateAssignmentWeights(Category category,
			Boolean enforceEqualWeighting, List<Assignment> assignments) {
		...
		if (assignments != null) {
			for (Assignment assignment : assignments) {
				...
				boolean isWeighted = assignment.isUnweighted() == null ? true
						: !assignment.isUnweighted().booleanValue();
				boolean isExtraCredit = assignment.isExtraCredit() == null ? false
						: assignment.isExtraCredit().booleanValue();
		...
		if (enforceEqualWeighting != null
				&& enforceEqualWeighting.booleanValue()
				&& !category.isEqualWeightAssignments().equals(Boolean.TRUE)) {
		...
		if (doRecalculate) {
			Double newWeight = gradeCalculations
					.calculateEqualWeight(weightedCount);
			if (assignments != null) {
				for (Assignment assignment : assignments) {
					boolean isRemoved = assignment.isRemoved();
					boolean isWeighted = assignment.isUnweighted() == null ? true
					...
					if (!isRemoved && isWeighted) {
						if (isExtraCredit)
							...
							Assignment persistAssignment = gbService
									.getAssignment(assignment.getId());
	...
	private StudentModel refreshLearnerData(Gradebook gradebook,
			StudentModel student, Assignment assignment,
			List<AssignmentGradeRecord> assignmentGradeRecords) {
		...
		for (AssignmentGradeRecord record : assignmentGradeRecords) {
			...
			String dropProperty = concat(String.valueOf(aId),
					StudentModel.DROP_FLAG);
			String excuseProperty = concat(String.valueOf(aId),
					StudentModel.EXCUSE_FLAG);
			boolean isDropped = record.isDropped() != null
					&& record.isDropped().booleanValue();
			boolean isExcluded = record.isExcluded() != null
					&& record.isExcluded().booleanValue();
		...
		String commentedProperty = assignment.getId()
				+ StudentModel.COMMENTED_FLAG;
		if (gbService.isStudentCommented(student.getIdentifier(), assignment
				.getId()))
	...
	private AssignmentGradeRecord scoreItem(Gradebook gradebook,
		...
		if (!isUserAbleToGrade)
			throw new InvalidInputException(
					"You are not authorized to grade this student for this item.");
		...
		if (gradebook.getGrade_type() == GradebookService.GRADE_TYPE_POINTS && value != null ) {
			...
			if (value.compareTo(Double.valueOf(100d)) > 0) 
				...
				throw new InvalidInputException(
						"This grade cannot be less than "
								+ DataTypeConversionUtil
										.formatDoubleAsPointsString(0d) + "%");
		}

		if (!includeExcluded && assignmentGradeRecord.isExcluded() != null
				&& assignmentGradeRecord.isExcluded().booleanValue())
			throw new InvalidInputException(
					"The student has been excused from this assignment. It is no longer possible to assign him or her a grade.");
		...
		case GradebookService.GRADE_TYPE_POINTS:
			...
			break;
		case GradebookService.GRADE_TYPE_PERCENTAGE:
			BigDecimal pointsEarned = gradeCalculations
					.getPercentAsPointsEarned(assignment, value);
			Double pointsEarnedDouble = pointsEarned == null ? null : Double
					.valueOf(pointsEarned.doubleValue());
	...
	private void verifyUserDataIsUpToDate(Site site, String[] learnerRoleKeys) {
		...
		int totalUsers = gbService.getFullUserCountForSite(siteId, null,
				learnerRoleKeys);
		int dereferencedUsers = gbService.getDereferencedUserCountForSite(
				siteId, null, learnerRoleKeys);
		...
		// Obviously if the realm count has changed, then we need to update, but let's also do it if more than an hour has passe
		long ONEHOUR = 1000l * 60l * 60l;
		if (lastUpdate == null
				|| lastUpdate.getRealmCount() == null
				|| !lastUpdate.getRealmCount().equals(Integer.valueOf(diff))
				|| lastUpdate.getLastUpdate() == null
				|| lastUpdate.getLastUpdate().getTime() + ONEHOUR < new Date()
						.getTime()) {
			gbService.syncUserDereferenceBySite(siteId, null, findAllMembers(
					site, learnerRoleKeys), diff, learnerRoleKeys);
		}
	}
	...
	private ItemModel updateGradebookModel(ItemModel item) {
		...
		ActionRecord actionRecord = new ActionRecord(gradebook.getUid(),
				gradebook.getId(), EntityType.GRADEBOOK.name(),
		...
		boolean isReleaseGrades = DataTypeConversionUtil.checkBoolean(item
				.getReleaseGrades());
	 ...
	 * @throws InvalidInputException
	 */
	private ItemModel updateCategoryModel(ItemModel item)
			throws InvalidInputException {

		boolean isWeightChanged = false;

		Category category = gbService.getCategory(Long.valueOf(item
				.getIdentifier()));
		Gradebook gradebook = category.getGradebook();

		ActionRecord actionRecord = new ActionRecord(gradebook.getUid(),
				gradebook.getId(), EntityType.CATEGORY.name(),
				ActionType.UPDATE.name());
		...
		try {
			...
			boolean originalExtraCredit = DataTypeConversionUtil
					.checkBoolean(category.isExtraCredit());
			boolean currentExtraCredit = DataTypeConversionUtil
					.checkBoolean(item.getExtraCredit());
			...
			isWeightChanged = isWeightChanged
					|| DataTypeConversionUtil.notEquals(newCategoryWeight,
							oldCategoryWeight);

			double w = newCategoryWeight == null ? 0d
					: ((Double) newCategoryWeight).doubleValue() * 0.01;

			boolean isEqualWeighting = DataTypeConversionUtil.checkBoolean(item
					.getEqualWeightAssignments());
			boolean wasEqualWeighting = DataTypeConversionUtil
					.checkBoolean(category.isEqualWeightAssignments());

			isWeightChanged = isWeightChanged
					|| isEqualWeighting != wasEqualWeighting;

			boolean isUnweighted = !DataTypeConversionUtil.checkBoolean(item
					.getIncluded());
			boolean wasUnweighted = DataTypeConversionUtil
					.checkBoolean(category.isUnweighted());
			...
			if (wasUnweighted && !isUnweighted && category.isRemoved())
				throw new InvalidInputException(
						"You cannot include a deleted category in grade. Please undelete the category first.");

			int oldDropLowest = category.getDrop_lowest();
			int newDropLowest = convertInteger(item.getDropLowest()).intValue();

			boolean isRemoved = DataTypeConversionUtil.checkBoolean(item
					.getRemoved());
			...
			if (hasCategories) {
				List<Category> categories = gbService.getCategories(gradebook
						.getId());
				// Business rule #2
			...
			if (w == 0d)
				category.setUnweighted(Boolean.TRUE);
			category.setEqualWeightAssignments(Boolean
					.valueOf(isEqualWeighting));
			...
			if (hasCategories) {
				List<Assignment> assignmentsForCategory = gbService
						.getAssignmentsForCategory(category.getId());

				if (isRemoved && !wasRemoved)
					businessLogic.applyRemoveChildItemsWhenCategoryRemoved(
							category, assignmentsForCategory);
	 ...
	 * 
	 * private int dropLowest; private boolean isEqualWeight;
	 * 
	 * public OnlyEqualWeightDropLowestRule(int dropLowest, boolean
	 * isEqualWeight) { this.dropLowest = dropLowest; this.isEqualWeight =
	 * isEqualWeight; }
	 * 
	 * public void isSatisfied() throws BusinessRuleException {
	 * 
	 * if (!isEqualWeight) { if (dropLowest > 0) { StringBuilder builder = new
	 * StringBuilder();builder.append(
	 * "Drop lowest is only valid for categories with equally weighted items. ")
	 * .
	 ...
	 * 
	 * private Long categoryId; private Long gradebookId; private String name;
	 * private List<Category> categories;
	 * 
	 * public NoDuplicateCategoryNamesRule(Long gradebookId, String name) {
	 * this.gradebookId = gradebookId; this.name = name; }
	 * 
	 * public NoDuplicateCategoryNamesRule(Long gradebookId, String name, Long
	 * categoryId) { this(gradebookId, name); this.categoryId = categoryId; }
	 * 
	 * public NoDuplicateCategoryNamesRule(Long gradebookId, String name, Long
	 * categoryId, List<Category> categories) { this(gradebookId, name,
	 * categoryId); this.categories = categories; }
	 * 
	 * public void isSatisfied() throws BusinessRuleException { if (categories
	 * == null) categories = gbService.getCategories(gradebookId);
	 * 
	 * if (categories != null) { for (Category c : categories) { if
	 * (!c.isRemoved() && c.getName() != null && name != null &&
	 * c.getName().trim().equalsIgnoreCase(name.trim())) { if (categoryId !=
	 * null && categoryId.equals(c.getId())) continue;
	 * 
	 * StringBuilder builder = new StringBuilder();
	 * builder.append("There is already an existing category called \""
	 * ).append(name).append("\" ").append("in this gradebook. ")
	 ...
	 * 
	 * private Long assignmentId; private Long gradebookId; private String name;
	 * 
	 * public NoDuplicateItemNamesRule(Long gradebookId, String name) {
	 * this.gradebookId = gradebookId; this.name = name; }
	 * 
	 * public NoDuplicateItemNamesRule(Long gradebookId, String name, Long
	 * assignmentId) { this(gradebookId, name); this.assignmentId =
	 * assignmentId; }
	 * 
	 * public void isSatisfied() throws BusinessRuleException { List<Assignment>
	 * assignments = gbService.getAssignments(gradebookId);
	 * 
	 * if (assignments != null) { for (Assignment a : assignments) { if
	 * (!a.isRemoved() && a.getName() != null && name != null &&
	 * a.getName().trim().equalsIgnoreCase(name.trim())) { if (assignmentId !=
	 * null && assignmentId.equals(a.getId())) continue;
	 * 
	 * StringBuilder builder = new StringBuilder();
	 * builder.append("There is already an existing item called \""
	 * ).append(name).append("\" ").append("in this gradebook. ")
	 * .append("Please enter a different name for the grade item.");
	 * 
	 * throw new BusinessRuleException(builder.toString()); } } } } }
	 * 
	 * private class NoDuplicateItemNamesWithinCategoryRule implements
	 * BusinessRule {
	 * 
	 * private Long assignmentId; private Long categoryId; private String name;
	 * 
	 * public NoDuplicateItemNamesWithinCategoryRule(Long categoryId, String
	 * name) { this.categoryId = categoryId; this.name = name; }
	 * 
	 * public NoDuplicateItemNamesWithinCategoryRule(Long categoryId, String
	 * name, Long assignmentId) { this(categoryId, name); this.assignmentId =
	 * assignmentId; }
	 * 
	 * public void isSatisfied() throws BusinessRuleException { List<Assignment>
	 * assignments = gbService.getAssignmentsForCategory(categoryId);
	 * 
	 * if (assignments != null) { for (Assignment a : assignments) { if
	 * (!a.isRemoved() && a.getName().trim().equalsIgnoreCase(name.trim())) { if
	 * (assignmentId != null && assignmentId.equals(a.getId())) continue;
	 * 
	 * StringBuilder builder = new StringBuilder();
	 * builder.append("There is already an existing item called \""
	 * ).append(name).append("\" ").append("in this category. ")
	 * .append("Please enter a different name for the grade item.");
	 * 
	 * throw new BusinessRuleException(builder.toString()); } } } } }
	 * 
	 ...
	 * 
	 * private Category category; private boolean includeInGrade;
	 * 
	 * public RecalculateEqualWeightingRule(Category category, boolean
	 * includeInGrade) { this.category = category; this.includeInGrade =
	 * includeInGrade; }
	 * 
	 * public void isSatisfied() throws BusinessRuleException { boolean
	 * hasEqualWeighting =
	 * DataTypeConversionUtil.checkBoolean(category.isEqualWeightAssignments());
	 * if (hasEqualWeighting) recalculateAssignmentWeights(category.getId(),
	 * null);
	 * 
	 * 
	 * } }
	 ...
	 * 
	 * private Long assignmentId; private Double newPoints; private Double
	 * oldPoints;
	 * 
	 * public RecalculatePointsRule(Long assignmentId, Double newPoints, Double
	 * oldPoints) { this.assignmentId = assignmentId; this.newPoints =
	 * newPoints; this.oldPoints = oldPoints; }
	 * 
	 * public void isSatisfied() throws BusinessRuleException {
	 * 
	 * if (newPoints != null && oldPoints != null &&
	 * newPoints.compareTo(oldPoints) != 0) {
	 ...
	 * 
	 * public MustIncludeCategoryRule(Long categoryId) { this.categoryId =
	 * categoryId; }
	 * 
	 * public void isSatisfied() throws BusinessRuleException { if (categoryId
	 * == null) throw new
	 * BusinessRuleException("You must select a category to group this item under."
	 * ); }
	 * 
	 * }
	 * 
	 ...
	 * 
	 * if (assignments != null) { for (Assignment assignment : assignments) {
	 * assignment.setRemoved(true); gbService.updateAssignment(assignment); } }
	 * 
	 * }
	 * 
	 * 
	 * }
	 * 
	 * private class RemoveEqualWeightingWhenItemWeightChangesRules implements
	 * BusinessRule {
	 * 
	 * private Category category; private Double oldAssignmentWeight; private
	 * Double newAssignmentWeight; private boolean isExtraCredit; private
	 * boolean isUnweighted; private boolean wasUnweighted;
	 * 
	 * public RemoveEqualWeightingWhenItemWeightChangesRules(Category category,
	 * Double oldAssignmentWeight, Double newAssignmentWeight, boolean
	 * isExtraCredit, boolean isUnweighted, boolean wasUnweighted) {
	 * this.category = category; this.oldAssignmentWeight = oldAssignmentWeight;
	 * this.newAssignmentWeight = newAssignmentWeight; this.isExtraCredit =
	 * isExtraCredit; this.isUnweighted = isUnweighted; this.wasUnweighted =
	 * wasUnweighted; }
	 * 
	 * public void isSatisfied() throws BusinessRuleException { if
	 * (!isUnweighted && !wasUnweighted) { if (oldAssignmentWeight == null ||
	 * !oldAssignmentWeight.equals(newAssignmentWeight)) {
	 * 
	 * if (!isExtraCredit && category.isEqualWeightAssignments() != null &&
	 * category.isEqualWeightAssignments().booleanValue()) { Category
	 * editCategory = gbService.getCategory(category.getId());
	 ...
	 * 
	 * private boolean isUnweighted; private boolean isAssignmentRemoved;
	 * private boolean isCategoryRemoved;
	 * 
	 * public CannotIncludeDeletedItemRule(boolean isAssignmentRemoved, boolean
	 * isCategoryRemoved, boolean isUnweighted) { this.isAssignmentRemoved =
	 * isAssignmentRemoved; this.isCategoryRemoved = isCategoryRemoved;
	 * this.isUnweighted = isUnweighted; }
	 * 
	 * public void isSatisfied() throws BusinessRuleException { if
	 * (!isUnweighted) { if (isCategoryRemoved) throw newBusinessRuleException(
	 * "You cannot include a grade item whose category has been deleted in grading. Please undelete the category first."
	 * );
	 * 
	 * if (isAssignmentRemoved) throw newBusinessRuleException(
	 * "You cannot include a deleted grade item in grading. Please undelete the grade item first."
	 * ); } }
	 * 
	 * }
	 * 
	 * 
	 * private class CannotIncludeItemFromUnincludedCategoryRule implements
	 * BusinessRule {
	 * 
	 * private boolean isCategoryIncluded, isItemIncluded, wasItemIncluded;
	 * 
	 * public CannotIncludeItemFromUnincludedCategoryRule(boolean
	 * isCategoryIncluded, boolean isItemIncluded, boolean wasItemIncluded) {
	 * this.isCategoryIncluded = isCategoryIncluded; this.isItemIncluded =
	 * isItemIncluded; this.wasItemIncluded = wasItemIncluded; }
	 * 
	 * public void isSatisfied() throws BusinessRuleException { if
	 * (!isCategoryIncluded) { if (isItemIncluded && !wasItemIncluded) throw new
	 * BusinessRuleException(
	 * "You cannot include a grade item whose category is not included in grading. Please include the category first."
	 * ); } }
	 * 
	 * }
	 */
	...
	static final Comparator<EnrollmentRecord> ENROLLMENT_NAME_COMPARATOR = new Comparator<EnrollmentRecord>() {
		public int compare(EnrollmentRecord o1, EnrollmentRecord o2) {
			return o1.getUser().getSortName().compareToIgnoreCase(
					o2.getUser().getSortName());
		}
	};
	...
	static final Comparator<UserRecord> SECTION_TITLE_COMPARATOR = new Comparator<UserRecord>() {
		public int compare(UserRecord o1, UserRecord o2) {
			...
			return o1.getSectionTitle().compareToIgnoreCase(
					o2.getSectionTitle());
		}
	};
